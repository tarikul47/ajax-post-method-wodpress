 ...
 * Ajax action here 
 */
function contact_us_ajax_call()
{

	$data = [];
	wp_parse_str($_POST['form-data'], $data);
	echo '<pre>';
	print_r($data);
	//wp_send_json_success($data);
}
add_action('wp_ajax_custom_contact2', 'contact_us_ajax_call');


function ajax_meal_call()
{
	...
	//$data = [];
	//wp_parse_str($_POST['fdatas'], $data);
	print_r($data);

	//wp_send_json_success('success');
}
add_action('wp_ajax_contact', 'ajax_meal_call');
add_action('wp_ajax_nopriv_contact', 'ajax_meal_call');
